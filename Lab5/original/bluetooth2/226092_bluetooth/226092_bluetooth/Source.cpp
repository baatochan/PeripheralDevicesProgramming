...
int main(int argc, char **args)
{
	...
	BluetoothRemoveDevice(&m_device_info[wybor].Address);

	Sleep(5000);

	HRESULT status = BluetoothAuthenticateDeviceEx(NULL, NULL, &m_device_info[wybor], NULL, MITMProtectionNotRequired);
	...
	if (sck == INVALID_SOCKET) {
		...
		sadr.btAddr = m_device_info[wybor].Address.ullLong;
		...
		wprintf(L"\n\tDevice %d:\r\n", wybor);
		wprintf(L"  \tInstance Name: %s\r\n", m_device_info[wybor].szName);
		wprintf(L"  \tConnected: %s\r\n", m_device_info[wybor].fConnected ? L"true" : L"false");
